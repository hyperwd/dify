...
COMPOSE_TEMPLATE="$DEPLOY_DIR/docker-compose-dynamic.yaml"
...
generate_compose_file() {
    ...
    if [[ -f "$COMPOSE_TEMPLATE" ]]; then
        ...
        sed "s#SERVER_IP_HERE#$server_ip#g" "$COMPOSE_TEMPLATE" > "$temp_file"
        ...
        rm "$temp_file"
        log_success "Docker Compose配置文件生成完成: $compose_file"
    else
        log_error "找不到模板文件: $COMPOSE_TEMPLATE"
        exit 1
    fi
