...
COMPOSE_FILE="$DEPLOY_DIR/docker-compose-brand.yaml"
...
show_access_info() {
    ...
    echo "  更新服务: cd $DEPLOY_DIR && docker compose -f $COMPOSE_FILE pull && docker compose -f $COMPOSE_FILE up -d"
    echo
...
# 显示帮助信息
show_help() {
    echo "Dify AlmaLinux 9 品牌定制部署脚本"
    echo
    echo "用法: $0 [选项]"
    echo
    echo "选项:"
    echo "  -h, --help     显示帮助信息"
    echo "  -b, --build    强制重新构建品牌镜像"
    echo "  -p, --pull     仅拉取官方镜像"
    echo "  -e, --env-only 仅设置环境配置"
    echo "  -w, --web-only 仅构建品牌前端"
    echo
    echo "示例:"
    echo "  $0                # 完整部署"
    echo "  $0 -b             # 强制重建并部署"
...
main() {
    ...
    if [[ "$force_build" == true ]]; then
        ...
        # 检查品牌镜像是否存在
        if ! docker images | grep -q "almalinux9.*brand\|dify-web.*brand"; then
            build_brand_image
        else
            log_info "品牌前端镜像已存在，跳过构建 (使用 -b 强制重新构建)"
        fi
    fi
